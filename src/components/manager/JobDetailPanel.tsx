...
export function JobDetailPanel({ job, onBack, onEdit }: JobDetailPanelProps) {
  ...
  const loadDetail = async () => {
    const [assignRes, loadsRes] = await Promise.all([
      supabase
        .from('job_assignments')
        .select('*, vehicles(registration, vehicle_type)')
        .eq('job_id', job.id)
        .order('created_at'),
      supabase
        .from('loads')
    ...
    if (assignRes.data) setAssignments(assignRes.data as AssignmentWithVehicle[]);
  ...
  const totalCompleted = Object.values(loadsByAssignment)
    .flat()
    .filter((l) => l.status === 'completed').length;
  const totalAssigned = assignments.reduce((s, a) => s + a.loads_assigned, 0);
  const hasActiveLoad = Object.values(loadsByAssignment)
    .flat()
    .some((l) => IN_PROGRESS.includes(l.status));
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {job.direction && (
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
              job.direction === 'import' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'
            }`}>
        ...
        <div className="flex gap-5 text-sm border-t border-slate-100 pt-3">
          ...
          {job.rate_per_load && (
            <div>
              ...
              <p className="font-bold text-slate-800">£{Number(job.rate_per_load).toFixed(2)}/load</p>
            </div>
          )}
          {job.start_date && (
            <div>
      ...
      {/* Assignments */}
      <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Vehicles</p>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {assignments.map((a) => {
            ...
            const progressPct = a.loads_assigned > 0 ? (completed / a.loads_assigned) * 100 : 0;
              ...
              <div key={a.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="ml-auto text-sm font-bold text-slate-700">
                    {completed}/{a.loads_assigned}
                    <span className="text-xs font-normal text-slate-400 ml-1">loads</span>
                  </div>
                </div>

                {/* Progress bar */}
                <div className="h-1 bg-slate-100 mx-4 rounded-full mb-2">
                  <div
                ...
                <div className="pb-1">
                  {loads.map((load) => {
                    ...
                    return (
                      <div
                        key={load.id}
                        className="px-4 py-2 flex items-center gap-3 border-t border-slate-50 first:border-0"
                      >
